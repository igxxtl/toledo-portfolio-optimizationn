...
"""
train_xgb.py — Regressor XGBoost para o vetor Q (Black-Litterman)
-----------------------------------------------------------------
...
try:
    from .config import DATA_DAILY_DIR, XGB_OUTPUT_DIR
except ImportError:
    from config import DATA_DAILY_DIR, XGB_OUTPUT_DIR

DATA_DIR = DATA_DAILY_DIR
OUT_DIR = XGB_OUTPUT_DIR
...
def carregar_df(fp: Path) -> pd.DataFrame:
    ...
    if faltando:
        raise ValueError(f"Colunas ausentes — rode prepare_features.py antes: {faltando}")
...
def main() -> None:
    ...
    if not csvs:
        print(f"Nenhum CSV de ativo em {DATA_DIR}. Rode prepare_features.py primeiro.")
        sys.exit(1)
    if TICKERS != "all":
        alvo = set(TICKERS)
